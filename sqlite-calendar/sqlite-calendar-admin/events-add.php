...
if($year.$month.str_pad($day, 2, '0', STR_PAD_LEFT) < $today_yea.$today_mon.$today_day || empty($year) || empty($month) || empty($day))
{
header("Location: index.php?status=2");
exit;
}
...
$stmt = $db->prepare("INSERT INTO Calendar (TimeStamp, EVENT) VALUES (:TimeStamp ,:Content)");
$fulltimestamp = $year . "-" . $month . "-" . str_pad($day, 2, '0', STR_PAD_LEFT);
$stmt->bindValue(':TimeStamp', $fulltimestamp);
...
<div class="content">
<br>
Add an event for <?php echo dateName($year. "-" . $month . "-" . $day);
